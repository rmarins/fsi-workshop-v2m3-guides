...
These features are important for any distributed applications built on top of Kubernetes/Openshift. We use a different set of microservices that you will be building and deploying.
Our application is called the _bankdemo microservice_ developed prior to this lab (i.e Account, Transaction).

[WARNING]
====
If you have already deployed the _transaction_ and _account_ microservices from Module 1, you can skip this step and move to section "1. Enabling automatic sidecar injection"
====

You should deploy the bankdemo application and microservices by executing the following shell scripts in CodeReady Workspaces Terminal:

The following script will deploy the transaction service:

[source, shell, role="copypaste"]
----
sh $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/istio/scripts/deploy-transaction.sh {{ USER_ID }}  && \
sh $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/istio/scripts/deploy-account.sh {{ USER_ID }}
----

[WARNING]
====
It sometimes takes time to create a new build image for network latency in OpenShift. So if you got failed to deploy account-service with *Error from server (NotFound): services "account-springboot" not found*. Please try again with delay via the following command:
====

[source, sh, role="copypaste"]
----
sh $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/istio/scripts/deploy-transaction.sh {{ USER_ID }}  && \
sh $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/istio/scripts/deploy-account.sh {{ USER_ID }} 3m
----

Wait for the commands to complete. This will build and deploy the _transaction_ and _account_ components into their own namespaces. They won’t automatically get Istio sidecar proxy containers yet, but you’ll add that in the next step!
...
==== Confirm `transaction` and `account` services are running

First, confirm you have running account service and associated database with the following command:

[source,sh,role="copypaste"]
----
oc get pods -n {{USER_ID}}-account --field-selector status.phase=Running
----
...
account-database-1-p5hcd     1/1     Running   0          6m4s
account-springboot-1-jdrfs   1/1     Running   0          5m
----

Do the same for transaction:

[source,sh,role="copypaste"]
----
oc get pods -n {{USER_ID}}-transaction --field-selector status.phase=Running
----
...
----
NAME                         READY   STATUS    RESTARTS   AGE
transaction-1-87smf          1/1     Running   0          3m12s
transaction-database-1-6tvxs 1/1     Running   0          5m16s
----
...
Rather than manually adding the annotations necessary to inject istio sidecars, run the following commands to add the annotations which will trigger a sidecar to be injected into our _transaction_ and _account_ microservices, as well as their associated databases.

First, do the databases and wait for them to be re-deployed:
[source,sh,role="copypaste"]
----
oc patch dc/transaction-database -n {{USER_ID}}-transaction --type='json' -p '[{"op":"add","path":"/spec/template/metadata/annotations", "value": {"sidecar.istio.io/inject": "'"true"'"}}]' && \
oc patch dc/account-database -n {{USER_ID}}-account --type='json' -p '[{"op":"add","path":"/spec/template/metadata/annotations", "value": {"sidecar.istio.io/inject": "'"true"'"}}]' && \
oc rollout status -w dc/transaction-database -n {{USER_ID}}-transaction && \
oc rollout status -w dc/account-database -n {{USER_ID}}-account
----
...
[source,sh,role="copypaste"]
----
oc patch dc/transaction -n {{USER_ID}}-transaction --type='json' -p '[{"op":"add","path":"/spec/template/metadata/annotations", "value": {"sidecar.istio.io/inject": "'"true"'"}}]' && \
oc rollout latest dc/transaction -n {{USER_ID}}-transaction && \
oc patch dc/account-springboot -n {{USER_ID}}-account --type='json' -p '[{"op":"add","path":"/spec/template/metadata/annotations", "value": {"sidecar.istio.io/inject": "'"true"'"}}]' && \
oc rollout status -w dc/transaction -n {{USER_ID}}-transaction && \
oc rollout status -w dc/account-springboot -n {{USER_ID}}-account
----

This should also take about 1 minute to finish. When it's done, verify that the `transaction-database` is running with 2 pods (`2/2` in the `READY` column) with this command:
...
----
NAME                         READY   STATUS    RESTARTS   AGE
transaction-2-2p9bx          2/2     Running   0          65s
transaction-database-2-4rzlh 2/2     Running   0          82s
----

Do the same for the account and confirm they also show the "2/2" pods running:
...
----
NAME                         READY   STATUS    RESTARTS   AGE
account-database-2-djmbd     2/2     Running   0          39m
account-springboot-2-x7cf6   2/2     Running   0          39m
----

[WARNING]
====
It may take a minute or two before the `transaction` and `account` services are recognized and brought into the mesh.
====

Next, let's create a virtual service to send incoming traffic to the account. Open the empty `account-default.yaml` file in _account/rules_ directory to copy the following _VirtualService_ into the empty file using CodeReady Workspaces:
...
spec:
  ...
  http:
    - match:
        - uri:
            exact: /services/accounts
        - uri:
            exact: /
      route:
        - destination:
            host: account-springboot
            port:
              number: 8080
----
...
[source,sh,role="copypaste"]
----
oc create -f $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/account/rules/account-default.yaml -n {{ USER_ID }}-account
----

Access the http://istio-ingressgateway-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[Account Service Page^] and ensure it should look something like:

image::account-ui-gateway.png[account, 700]

[NOTE]
====
It takes a few seconds to reconcile _istio ingress_ with the _gateway_ and _virtual service_. Leave this page open as the _Account UI browser_ creates traffic (every 2 seconds) between services, which is useful for testing.
====

Ensure if we injected _side car_ to each pods. Access the https://kiali-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/console/graph/namespaces/?edges=noEdgeLabels&graphType=versionedApp&namespaces={{ USER_ID }}-account%2C{{ USER_ID }}-transaction&unusedNodes=false&injectServiceNodes=true&duration=60&pi=15000&layout=dagre[Kiali Graph page^] and verify that *{{ USER_ID }}-transaction*, *{{ USER_ID }}-account* are selected _Namespaces_ then enable *Traffic Animation* in the _Display_ drop-down to see animated traffic flow from _Account service_ to _Transaction service_:

image::kiali_graph_sidecar.png[istio, 700]

You can see the incoming traffic to the account service along with traffic going to both the account and transaction databases along each branch. This mirrors what we would expect - when you access the account frontend, a call is made to the account backend, which in turn access the transaction and combines it with account data and returns the result for display.
...
To test our application microservices for resiliency, we will inject a failure in *50%* of the requests to the _transaction_ service, causing the service to appear to fail (and return `HTTP 5xx` errors) half of the time.

Open the empty `transaction-default.yaml` file in the _transaction/rules_ directory and copy the following into the file:
...
Delete the gateway to direct route account that was setup earlier with:

[source,sh,role="copypaste"]
----
oc delete -f $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/account/rules/account-default.yaml -n {{ USER_ID }}-account
----

Create the new VirtualService to direct traffic to the transaction service by running the following command via CodeReady Workspaces Terminal:

[source,sh,role="copypaste"]
----
oc create -f $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/transaction/rules/transaction-default.yaml -n {{ USER_ID }}-transaction
----

Now, you can test if the transaction service works correctly via accessing the http://istio-ingressgateway-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[bankdemo Transaction page^]. If you still see _bankdemo Account_ then reload the page to see _bankdemo Transaction_ with kbd:[CTRL+F5] (or kbd:[Command+Shift+R] on Mac OS).

image::transaction-ui-gateway.png[fault-injection, 700]

Let’s inject a failure (_500 status_) in *50%* of requests to _transaction_ microservices. Edit _transaction-default.yaml_ as below.

Open the empty `transaction-vs-fault.yaml` file in _transaction/rules_ directory and copy the following codes.
...
Before creating a new *transaction-fault VirtualService*, we need to delete the existing transaction-default virtualService. Run the following command via CodeReady Workspaces Terminal:

[source,sh,role="copypaste"]
----
oc delete virtualservice/transaction-default -n {{ USER_ID }}-transaction
----
...
[source,sh,role="copypaste"]
----
oc create -f $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/transaction/rules/transaction-vs-fault.yaml -n {{ USER_ID }}-transaction
----

Let’s find out if the fault injection works corectly via accessing the http://istio-ingressgateway-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[bankdemo Transaction page^] once again. You will see that the *Status* of bankdemo Transaction continues to change between *DEAD* and *OK*:

image::transaction-dead-ok.png[fault-injection, 700]

Back on the https://kiali-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/console/graph/namespaces/?edges=noEdgeLabels&graphType=versionedApp&namespaces={{ USER_ID }}-account%2C{{ USER_ID }}-transaction&unusedNodes=false&injectServiceNodes=true&duration=60&pi=15000&layout=dagre[Kiali Graph page^] and you will see `red` traffic from _istio-ingressgateway_ as well as around 50% of requests are displayed as _5xx_ on the right side, _HTTP Traffic_. It may not be _exactly_ 50% since some traffic is coming from the account and ingress gateway at the same time, but it will approach 50% over time.
...
image::transaction-vs-error-kiali.png[fault-injection,700]

Let’s now add a 5 second delay for the `transaction` service.

Open the empty `transaction-vs-fault-delay.yaml` file in _transaction/rules_ directory and copy the following code into it:
...
Delete the existing transaction-fault VirtualService in CodeReady Workspaces Terminal:

[source,sh,role="copypaste"]
----
oc delete virtualservice/transaction-fault -n {{ USER_ID }}-transaction
----
...
[source,sh,role="copypaste"]
----
oc create -f $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/transaction/rules/transaction-vs-fault-delay.yaml -n {{ USER_ID }}-transaction
----

Go to the *Kiali Graph* you opened earlier and you will see that the `green` traffic from _istio-ingressgateway_ is delayed for requests coming from transaction service. Note that you need to check *Traffic Animation* in the _Display_ select box.
...
image::transaction-vs-delay-kiali.png[fault-injection,700]

Click on the "edge" (the line between `istio-ingressgateway` and `transaction`) and then scroll to the bottom of the right-side graph showing the _HTTP Request Response Time_. Hover over the black _average_ data point to confirm that the average response time is about 5000ms (5 seconds) as expected:

image::5sdelay.png[delay, 800]

If the Transaction’s front page was set to correctly handle delays, we expect it to load within approximately 5 seconds. To see the web page response times, open the Developer Tools menu in IE, Chrome or Firefox (typically, key combination kbd:[CTRL+SHIFT+I] or kbd:[CMD+ALT+I] on a Mac), select the `Network` tab, and reload the transaction web page.

You will see and feel that the webpage loads in about 5 seconds:

image::transaction-webui-delay.png[Delay, 700]
...
[source,sh,role="copypaste"]
----
oc delete virtualservice/transaction-fault-delay -n {{ USER_ID }}-transaction && \
oc create -f $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/transaction/rules/transaction-default.yaml -n {{ USER_ID }}-transaction
----

Also, close the tabs in your browser for the Transaction and Account services to avoid unnecessary load, and stop the endless `for` loop you started in the beginning of this lab in CodeReady by closing the Terminal window that was running it.

=== 3. Enable Circuit Breaker

In this step, you will configure a circuit Breaker to protect the calls to `Transaction` service. If the `Transaction` service gets overloaded due to call volume, Istio will limit future calls to the service instances to allow them to recover.
...
Let’s add a circuit breaker to the calls to the *Transaction service*. Instead of using a _VirtualService_ object, circuit breakers in Istio are defined as _DestinationRule_ objects. DestinationRule defines policies that apply to traffic intended for a service after routing has occurred. These rules specify configuration for load balancing, connection pool size from the sidecar, and outlier detection settings to detect and evict unhealthy hosts from the load balancing pool.

Open the empty `transaction-cb.yaml` file in _transaction/rules_ directory and add this code to the file to enable circuit breaking when calling the Transaction service:
...
To see this, open the https://grafana-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/d/LJ_uJAvmk/istio-service-dashboard?orgId=1&refresh=10s&var-service=transaction.{{ USER_ID }}-transaction.svc.cluster.local&var-srcns=All&var-srcwl=All&var-dstns=All&var-dstwl=All[Istio Service Dashboard^] in Grafana and ensure to see _Client Success Rate(non-5xx responses)_ of transaction service is no longer at 100%:
...
image::transaction-circuit-breaker-grafana.png[circuit-breaker, 700]
...
image::transaction-circuit-breaker-kiali.png[circuit-breaker, 700]
...
[source,sh,role="copypaste"]
----
oc delete destinationrule/transaction-cb -n {{ USER_ID }}-transaction && \
oc delete virtualservice/transaction-default -n {{ USER_ID }}-transaction && \
oc create -f $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/account/rules/account-default.yaml -n {{ USER_ID }}-account
----
...
In this step, you will learn how to enable authentication. You will secure the _Account_ endpoint. We will use JWT with Red Hat Single Sign On which is part of the Red Hat Runtimes.
...
We will deploy RH-SSO in a new project. Go to the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-account[Topology View^], click on *Create Project*:
...
Click on *From Account* in Topology view:

image::create_project_fc.png[rhsso, 700]

Type `ccn-sso` in search box and click on *CCN + Red Hat Single Sign-On 7.4 on OpenJDK + PostgreSQL* account. Then click on *Instantiate Template*:

image::account_rhsso.png[rhsso, 700]
...
image::account_rhsso_detail.png[rhsso, 700]
...
image::rhsso_istio_realm.png[sso, 900]
...
image::rhsso_clients.png[sso, 900]

Input `ccn-cli` in _Client ID_ field and click on *Save*.

image::rhsso_clients_create.png[sso, 900]
...
image::rhsso_clients_settings.png[sso, 900]
...
image::rhsso_roles.png[sso, 900]

Input `ccn_auth` in _Role Name_ field and click on *Save*.

image::rhsso_roles_create.png[sso, 900]
...
image::rhsso_istio_users_details.png[sso, 900]
...
In CodeReady, open the empty *ccn-auth-config.yml* file in _account/rules_ directory to copy the following policy.
...
[source,sh,role="copypaste"]
----
oc create -f $CHE_PROJECTS_ROOT/fsi-workshop-v2m3-labs/account/rules/ccn-auth-config.yaml -n {{ USER_ID }}-account
----

Now you can’t access the account service without authentication of RH-SSO. You confirm it using the following curl command in CodeReady Workspaces Terminal:
...
image::rhsso_call_account_noauth.png[sso, 900]
...
[source,sh,role="copypaste"]
----
curl -H "Authorization: Bearer $TOKEN" http://istio-ingressgateway-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/services/accounts | jq
----
...
...
  {
    ...
    "beginBalance": 166.08,
    "creditLine": 5000,
    "beginBalanceTimeStamp": "2019-07-23",
    "accountId": "5007",
    "description": "Visa",
    "type": "Credit",
    "balancDate": "2019-10-27",
    "remainingCredit": 4783.05
  },
  {
    "balance": 57601.35,
    "beginBalance": 5433.89,
    "creditLine": 0,
    "beginBalanceTimeStamp": "2019-07-15",
    "accountId": "5008",
    "description": "Super Interest Account",
    "type": "Savings",
    "balancDate": "2019-10-28",
    "remainingCredit": 0
  }
]
----

Congratulations! You’ve integrated RH-SSO with Istio to protect service mesh traffic to the account service, without having to change the application at all. Istio can use Keycloak to authenticate service-to-service calls (also called "east-west" traffic).
