...
----
NAME                         READY   STATUS    RESTARTS   AGE
transaction-quarkus-1-87smf  1/1     Running   0          3m12s
...
[source,sh,role="copypaste"]
----
oc patch dc/transaction-quarkus -n {{USER_ID}}-transaction --type='json' -p '[{"op":"add","path":"/spec/template/metadata/annotations", "value": {"sidecar.istio.io/inject": "'"true"'"}}]' && \
oc rollout latest dc/transaction-quarkus -n {{USER_ID}}-transaction && \
...
oc rollout status -w dc/transaction-quarkus -n {{USER_ID}}-transaction && \
...
----
NAME                         READY   STATUS    RESTARTS   AGE
transaction-quarkus-2-2p9bx  2/2     Running   0          65s
...
To see this, open the https://grafana-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/d/LJ_uJAvmk/istio-service-dashboard?orgId=1&refresh=10s&var-service=transaction-quarkus.{{ USER_ID }}-transaction.svc.cluster.local&var-srcns=All&var-srcwl=All&var-dstns=All&var-dstwl=All[Istio Service Dashboard^] in Grafana and ensure to see _Client Success Rate(non-5xx responses)_ of transaction service is no longer at 100%:
